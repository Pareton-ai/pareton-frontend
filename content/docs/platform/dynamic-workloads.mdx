...
Set `sampling_rule` on a draft campaign. Type `hf_rows`, with `dataset`, `revision` (a git SHA, not `main`), `config`, `split`, `n_rows`, `n_prompts`, `max_tokens`, and `algo_version` (1).

A campaign with no `sampling_rule` pins one fixed trace. Every round of that campaign draws that same trace.
