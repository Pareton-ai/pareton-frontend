...
function EntryRow({
  ...
  return (
    // Same stretched-link row as RoundRow, including its Safari note: WebKit
    // ignores `position: relative` on a <tr>, so `transform` makes the row a
    // containing block and `clip-path` keeps the overlay inside it.
    <tr className="group relative border-t border-border/80 transition-colors [clip-path:inset(0)] [transform:translate(0)] hover:bg-accent-dim/30 focus-within:bg-accent-dim/30">
      <td className="whitespace-nowrap px-4 py-3.5 font-mono text-body sm:px-5">
        {entry.patch_hash === null ? (
          ...
          <Link
            ...
            className="relative z-10 text-foreground underline decoration-border underline-offset-4 transition-colors hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
          >
            {entry.role.replaceAll("_", " ")}
          </Link>
        )}
      ...
      <td className="px-3 py-3.5">
        {entry.patch_hash === null ? (
          ...
          <span className="flex items-center gap-2">
            <Link
              href={submissionHref(campaignId, entry.patch_hash)}
              className="relative z-10 font-mono text-body text-secondary underline decoration-border underline-offset-4 transition-colors hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
            >
              {truncateDigest(entry.patch_hash, 7, 5)}
            </Link>
            <CopyableMono
              value={entry.patch_hash}
              iconOnly
      ...
      <td className="px-3 py-3.5">
        {/* Fixed so the chip and its reason never reflow the columns. w-64
            rather than RoundRow's w-72: it buys back exactly the width the
            chevron column costs, keeping the table inside its grid cell. */}
        <span className="flex w-64 max-w-64 items-center gap-2">
      ...
      <td className="whitespace-nowrap px-3 py-3.5 font-mono text-body text-secondary">
        {/* The row opens the breakdown, so this link is the row overlay and
            carries no underline of its own: the score stays a plain number and
            the affordance is the row hover plus the chevron. It sits on the
            score because that is the claim the breakdown explains. */}
        <Link
    ...
    <section aria-label="Round entries" className="border border-border">
      ...
      {entries.length === 0 ? (
        ...
        <div className="overflow-x-auto contain-paint">
          {/* contain-paint keeps the transformed row overlays from expanding
              the page; the min-width table still scrolls inside this pane. */}
